...
function HeroCategories() {
  return (
    <div className="flex justify-center items-center h-screen bg-[#EDF1F1] border-b">
      <div>
  ...
  <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
              ...
              <a 
                href={item.link} 
                className="mt-2 text-2xl px-3 hover:text-primary duration-200 text-[#255640] text-center"style={{fontFamily:'Jacques Francois, serif'}}
              >
                {item.name}
              </a>
            </li>
