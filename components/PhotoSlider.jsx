...
const PhotoSlider = ({headerText="Current favourites!"}) => {
  ...
  const prevSlide = () => {
    setCurrentIndex((prevIndex) =>
      (prevIndex - 1 + (totalSlides - slidesToShow + 1)) % (totalSlides - slidesToShow + 1)
    );
  };
        ...
        {slides.map((slide, index) => (
          ...
          >
            <div className="h-96 relative hover:scale-105 transition-transform duration-300 cursor-pointer">
              <div className="h-3/4" style={{ backgroundColor: slide.top }}></div>
              <div className="h-1/4" style={{ backgroundColor: slide.bottom }}></div>
